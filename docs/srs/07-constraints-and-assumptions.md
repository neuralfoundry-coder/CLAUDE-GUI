...
- 단, 단일 사용자가 여러 브라우저 탭에서 서로 다른 프로젝트를 독립적으로 작업하는 것은 지원한다 (`browserId` 기반 멀티 탭 독립 프로젝트, FR-1500).
...
- 각 브라우저 탭은 `sessionStorage` 기반 `browserId`(UUID)를 통해 독립적인 프로젝트 상태를 유지한다.
- `sessionStorage`는 탭 간 공유되지 않으므로, 새 탭·복제 탭은 항상 고유한 `browserId`를 부여받는다.
- 서버는 `BrowserSessionRegistry`를 통해 `browserId`별 프로젝트 루트를 관리하며, 연결 해제 후 30분 경과 시 해당 세션을 GC한다.

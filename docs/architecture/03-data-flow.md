...
**관련 FR**: FR-501, FR-502, FR-504, FR-505, FR-518

### 3.1.1 인텐트 감지 및 프롬프트 주입 흐름

슬라이드 등 콘텐츠 생성 요청 시, 클라이언트가 의도를 감지하고 서버가 시스템 프롬프트를 주입하는 흐름이다.
...
- 사용자 UI에는 원본 메시지만 표시된다 (`pushUserMessage(prompt)`).
- 시스템 프롬프트 주입은 서버 측에서만 수행되므로 클라이언트에 노출되지 않는다.
- 의도 감지 실패 시 일반 쿼리로 fallback한다.

**관련 FR**: FR-1201, FR-1202, FR-1203, FR-1204

### 3.1.2 슬래시 명령어 처리 흐름

사용자가 `/`로 시작하는 명령어를 입력할 때의 처리 흐름이다.
...
- 클라이언트 명령어(`/clear`, `/new`, `/usage`, `/context`, `/cost`, `/model`, `/help`)는 서버 통신 없이 즉시 처리된다.
- 패스스루 명령어(`/compact`, `/plan`, `/review`)는 입력 전체를 Claude CLI에 전달한다.
- 팝오버는 입력이 `/`로 시작하고 공백이 없는 동안만 표시되며, prefix 매칭으로 후보를 필터링한다.

**관련 FR**: FR-509, FR-516

### 3.1.3 파일/이미지 드래그 앤 드롭 흐름

사용자가 파일 또는 이미지를 Claude 채팅 패널에 드래그 앤 드롭하거나 클립보드에서 붙여넣기할 때의 처리 흐름이다.

```
사용자              Browser (React)              Server (REST)          파일시스템
  │                     │                             │                     │
  │ 1. 파일 드래그       │                             │                     │
  │──────────────────▶│                             │                     │
  │                     │ 2. hasFilePayload() 확인    │                     │
...
- 드래그 앤 드롭과 클립보드 붙여넣기(이미지)는 동일한 `uploadAndInsert()` 경로를 공유한다.
- 붙여넣기 이미지는 `paste-{timestamp}.{ext}` 파일명으로 자동 생성된다.
- 업로드 중 `AttachedFilesBar`에 스피너가 표시되고, 전송 버튼이 비활성화된다.
- `@` 참조는 CLI/SDK의 표준 문법 처리에 위임된다 (기존 FR-511과 동일).
...
PreviewRouter → 타입 감지
  ...
  ├── MD → MarkdownPreview
  ├── Image → ImagePreview
  └── Slides → SlidePreview (멀티페이지 세로 스크롤 + Edit 모드)
```

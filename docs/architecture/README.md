...
| ADR-016 | `ProjectContext` 싱글톤 기반 런타임 프로젝트 핫스왑 | 부팅 시 1회 고정이던 `PROJECT_ROOT`를 런타임에 교체. `src/lib/project/project-context.mjs`가 단일 source of truth이며 files/terminal/claude 핸들러와 `resolveSafe`가 모두 `getActiveRoot()`를 조회. 변경 시 파일 감시기(ADR-024 이후 `@parcel/watcher`) 재구독 + `project-changed` WS 브로드캐스트. 상태는 `~/.claudegui/state.json`에 영속화. |

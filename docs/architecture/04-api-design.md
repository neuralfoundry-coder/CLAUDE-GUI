...
현재 프로젝트 루트를 조회한다. `X-Browser-Id` 헤더가 있으면 해당 `browserId`의 프로젝트 루트를, 없으면 글로벌 싱글톤의 루트를 반환한다.

**요청 헤더**:
- `X-Browser-Id` (선택): 브라우저 탭 식별자 UUID
...
프로젝트 루트를 변경한다. `X-Browser-Id` 헤더가 있으면 해당 탭의 프로젝트 루트만 변경한다.
...
### 4.1.5 Claude CLI 패스스루 API

#### `POST /api/claude/cli`

Claude 채팅 패널의 슬래시 명령 중 `handler: 'cli'`로 표시된 항목과, 레지스트리에 없는 임의의 `/xxx` 입력을 로컬 `claude` 바이너리에 위임한다 (FR-516). 서버는 `execFile(<claude>, ['--print', command])`로만 실행하며 셸 인터폴레이션을 거치지 않는다.

**요청 헤더**:
- `Content-Type: application/json`
- `X-Browser-Id` (선택): cwd를 `browserSessionRegistry.getRoot(browserId)`로 결정. 없으면 `getActiveRoot()` 폴백.
...
**검증**:
- `command`은 `^/[A-Za-z][A-Za-z0-9_-]{0,63}(\s.*)?$` 정규식과 일치해야 한다 (`4400`).
- 길이 4096바이트 초과 시 거부 (`4400`).
- 활성 프로젝트 루트가 없으면 거부 (`4412`).
- 분당 호출 제한(`/lib/fs/rate-limit`) 적용 (`4429`).

**바이너리 탐색 순서**: Tauri-local 설치(`~/Library/Application Support/ClaudeGUI/node-prefix/bin/claude`) → `~/.local/bin/claude` → `/usr/local/bin/claude` → `/opt/homebrew/bin/claude` → PATH.

**응답** (성공 또는 비-제로 종료, 둘 다 stdout/stderr를 캡처해 200으로 반환):
```json
{
  "success": true,
  "data": {
...
**제한**:
- 30초 타임아웃 (`5408 / HTTP 504`)
- 출력 64KB 캡 (초과 시 잘리고 truncation 마커 추가)
- CLI 미설치 시 `5404 / HTTP 500`
...
- **`browserId` 파라미터**: 모든 WebSocket 엔드포인트는 `?browserId=<uuid>` 쿼리 파라미터를 선택적으로 받는다. 지정 시 해당 탭의 프로젝트 루트(ADR-027 `BrowserSessionRegistry`)가 사용되며, 미지정 시 글로벌 싱글톤(ADR-016)으로 폴백한다.
...
서버는 `browserId`가 있으면 `BrowserSessionRegistry`에서 해당 탭의 프로젝트 루트를, 없으면 `getActiveRoot()`를 `cwd`로 사용하여 새 `node-pty` 프로세스를 스폰한다 (기본 120×30). 클라이언트는 첫 attach 직후 실제 크기를 `resize` 제어 프레임으로 전송한다.
...
Claude Agent SDK와 연결된다. `?browserId=<uuid>` 쿼리 파라미터가 있으면 해당 탭의 프로젝트 루트를 `runQuery()`의 cwd로 사용한다.
...
`@parcel/watcher`가 수집한 파일 변경 이벤트를 브로드캐스트한다 (ADR-024). `?browserId=<uuid>` 쿼리 파라미터가 있으면 해당 탭의 프로젝트 루트에 대한 와처를 구독한다. `project-changed` 이벤트는 프로젝트를 변경한 `browserId`의 연결에만 전송된다.
